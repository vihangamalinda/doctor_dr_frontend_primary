...
function reFormatSelectorOptions(arr){
  if(!arr) return [];
  const createOptionData =(obj) =>({id: obj.id,name: `${obj.firstName} ${obj.lastName}`});
...
function AddRequestedAssistance() {
   ...
   const { addRequestedHelp, isAddingFeedback } = useRequestedHelpAddFeedBackById();
  ...
  return (
    ...
    <FormProvider {...methods}>
      <Form onSubmit={handleSubmit(onSubmit, onError)}>
        ...
        <Button variation="primary" size="large" type="submit" disabled={isAddingFeedback}> Add Feedback </Button>
        </Row>
      </Form>
    </FormProvider>
    </>
